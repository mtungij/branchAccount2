...
public function create_sponser($customer_id = null, $comp_id = null)
{
    ...
    $existingSponsorPassport = !empty($existingSponsor->passport_path)
      ? trim((string) $existingSponsor->passport_path)
      : '';
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      if (empty($sponser->passport_path) && $existingSponsorPassport !== '') {
        $sponser->passport_path = $existingSponsorPassport;
      }
        $this->load->view('officer/search_customer', [
            'customer' => $customer,
        'existing_passport' => $this->get_latest_customer_passport($customer_id),
        'existing_sponsor_passport' => $existingSponsorPassport,
        'sponser'  => $sponser
        ]);
        return;
    }
