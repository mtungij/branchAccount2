...
$this->load->model('queries');
        ...
        $received = $selected_blanch_id > 0
            ? $this->queries->get_received_loanBlanch($selected_blanch_id, null, $from, $to)
            : $this->queries->get_today_received_loan($comp_id, $from, $to);
        $total_receved = $this->queries->get_sumReceived_amount($comp_id, $selected_blanch_id > 0 ? $selected_blanch_id : null, $from, $to);
		$blanch = $this->queries->get_blanch($comp_id);
        ...
        //   print_r($received);
        //           exit();
        $this->load->view('admin/today_received',['received'=>$received,'total_receved'=>$total_receved,'blanch'=>$blanch,'total_principal_receive'=>$total_principal_receive,'total_interest'=>$total_interest,'selected_blanch_id'=>$selected_blanch_id,'filter_from'=>$from,'filter_to'=>$to]);
}
...
public function insert_loan_lecordData($comp_id,$customer_id,$loan_id,$blanch_id,$new_balance,$group_id,$trans_id,$restoration,$loan_aprove,$empl_id){
	...
	$empl_id = $this->session->userdata('empl_id');
    $this->load->model('queries');
    $p_method_input = (int) $this->input->post('p_method');
    if ($p_method_input > 0) {
        $method = $this->queries->get_account_by_transid($p_method_input);
        $method_name = $method ? strtolower(trim((string) $method->account_name)) : '';
        if ($method_name !== 'cash') {
            $this->form_validation->set_rules('wakala_name', 'Wakala', 'required|trim');
        }
    }
	...
	$this->form_validation->set_error_delimiters('<div class="text-danger">','</div>');
	   if ($this->form_validation->run()) {
	    ...
	    //       exit();
          ...
          $wakala_name = trim((string) $this->input->post('wakala_name', true));
...

{
	...
	public function get_blanch_receved(){
        ...
        $is_all_branches = ($blanch_id === 'all' || $blanch_id === '' || $blanch_id === null);

        if ($is_all_branches) {
            if ($empl_id == 'all') {
                $data_blanch = $this->queries->get_compReced_all($from, $to, $comp_id, $dep_status);
                $total_receve_blanch = $this->queries->get_comp_total_receved_all($from, $to, $comp_id, $dep_status);
            } else {
                $data_blanch = $this->queries->get_compReced($from, $to, $comp_id, $empl_id, $dep_status);
                $total_receve_blanch = $this->queries->get_comp_total_receved($from, $to, $comp_id, $empl_id, $dep_status);
            }
            $blanch_data = (object) array('blanch_name' => 'All Branches');
            $blanch_id = 'all';
        } else {
            if ($empl_id == 'all') {
    ...
    public function print_received_deposit($from = null,$to = null,$blanch_id = null,$empl_id = null,$dep_status = null){
		$this->load->model('queries');
		$comp_id = $this->session->userdata('comp_id');
        $compdata = $this->queries->get_companyData($comp_id);

        $from = !empty($from) ? $from : $this->input->get('from', true);
        $to = !empty($to) ? $to : $this->input->get('to', true);
        $blanch_id = ($blanch_id !== null && $blanch_id !== '') ? $blanch_id : $this->input->get('blanch_id', true);
        $empl_id = ($empl_id !== null && $empl_id !== '') ? $empl_id : $this->input->get('empl_id', true);
        if ($dep_status === null || $dep_status === '') {
            $dep_status = $this->input->get('dep_status', true);
        }
