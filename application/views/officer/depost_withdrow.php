...
<?php }
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200"><?= ($payisnulls->rem_debt) ? round($payisnulls->rem_debt, 2) : '0.00'; ?></td>
      ...
      <?php echo form_open("oficer/create_withdrow_balance/{$customer->customer_id}", ['id' => 'withdrawLoanForm']); ?>
...
<div class="p-4 sm:p-6">
  ...
  <div class="mt-6 flex justify-end items-center gap-x-2">
    ...
    <button type="submit" id="withdrawSubmitBtn" class="py-2 px-3 btn-primary-sm bg-cyan-600 hover:bg-cyan-700 text-white inline-flex items-center gap-2">
      <svg id="withdrawSubmitSpinner" class="hidden animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
      </svg>
      <span id="withdrawSubmitText">Gawa</span>
    </button>
  </div>
</div>
...
<div class="p-4 sm:p-6">
  ...
  <div class="mt-6 flex justify-end items-center gap-x-2">
    <button type="button" class="py-2 px-3 btn-secondary-sm"
      ...
      <button id="depositSubmitBtn" type="submit"
  class="py-2 px-4 btn-primary-sm bg-cyan-600 hover:bg-cyan-700 text-white inline-flex items-center gap-2">
  <svg id="depositSubmitSpinner" class="hidden animate-spin h-4 w-4 text-white"
  ...
  <span id="depositSubmitText">Weka</span>
</button>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const depositSpinner = document.getElementById('depositSubmitSpinner');
  const depositText = document.getElementById('depositSubmitText');
  const withdrawForm = document.getElementById('withdrawLoanForm');
  const withdrawBtn = document.getElementById('withdrawSubmitBtn');
  const withdrawSpinner = document.getElementById('withdrawSubmitSpinner');
  const withdrawText = document.getElementById('withdrawSubmitText');

  if (depositForm && depositBtn && depositSpinner && depositText) {
    depositForm.addEventListener('submit', function () {
      depositBtn.disabled = true;
      depositBtn.classList.add('opacity-70', 'cursor-not-allowed');
      depositSpinner.classList.remove('hidden');
      depositText.textContent = 'Inapakia...';
    });
  }

  if (withdrawForm && withdrawBtn && withdrawSpinner && withdrawText) {
    withdrawForm.addEventListener('submit', function () {
      withdrawBtn.disabled = true;
      withdrawBtn.classList.add('opacity-70', 'cursor-not-allowed');
      withdrawSpinner.classList.remove('hidden');
      withdrawText.textContent = 'Inapakia...';
    });
  }
});
</script>
