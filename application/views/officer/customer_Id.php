...
<script>
  const passportInput = document.getElementById('dropzone-file');
  const modal = document.getElementById('modal');
  const imageElement = document.getElementById('imagePreview');
  const closeModalBtn = document.getElementById('closeModalBtn');
  let cropper;

  function configurePassportInputForDevice() {
    ...
    if (isSmallTouchDevice) {
      passportInput.setAttribute('capture', 'environment');
    } else {
      passportInput.removeAttribute('capture');
    }
  }

  configurePassportInputForDevice();
  window.addEventListener('resize', configurePassportInputForDevice);
