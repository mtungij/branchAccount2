...
$today_loan_withdraw_url = base_url('admin/loan_withdrawal?from_date=' . urlencode($report_date) . '&to_date=' . urlencode($report_date) . '&blanch_id=' . (int) $selected_blanch_id);
...
<style>
	...
	.daily-report-filter .filter-field {
		min-width: 0;
	}

	.daily-report-filter .filter-field label {
		display: block;
		font-size: 12px;
		margin-bottom: 4px;
	}
	...
	.daily-report-filter .filter-actions {
		display: flex;
		gap: 10px;
		align-items: end;
	}

	.daily-report-filter .filter-actions a {
		height: 40px;
		padding: 0 14px;
		border-radius: 10px;
		display: inline-flex;
	...
	<div class="p-4 sm:p-6 lg:p-8">
		<div class="daily-report-wrap">
			<div class="daily-report-header">
				...
				<form method="get" action="<?php echo base_url('admin/daily_report'); ?>" class="daily-report-filter" id="daily-report-filter-form">
					<div class="filter-field">
						<label><?php echo $txt_date; ?></label>
						<input type="date" name="report_date" value="<?php echo htmlspecialchars($report_date); ?>" id="daily-report-date" />
					</div>
					<div class="filter-field">
						<label><?php echo $txt_branch; ?></label>
						<select name="blanch_id" id="daily-report-branch">
							<?php foreach ($blanch as $branch_row): ?>
								<option value="<?php echo (int) $branch_row->blanch_id; ?>" <?php echo $selected_blanch_id === (int) $branch_row->blanch_id ? 'selected' : ''; ?>>
									<?php echo htmlspecialchars($branch_row->blanch_name); ?>
								</option>
							<?php endforeach; ?>
						</select>
					</div>
					<div class="filter-actions">
						<a href="<?php echo base_url('admin/daily_report_pdf?report_date=' . urlencode($report_date) . '&blanch_id=' . (int) $selected_blanch_id); ?>"><?php echo $txt_download_pdf; ?></a>
					</div>
				</form>
			</div>
...
document.addEventListener('DOMContentLoaded', function () {
	const filterForm = document.getElementById('daily-report-filter-form');
	const dateInput = document.getElementById('daily-report-date');
	const branchSelect = document.getElementById('daily-report-branch');
	...
	if (dateInput) {
		dateInput.addEventListener('change', submitFilter);
	}

	if (branchSelect) {
		branchSelect.addEventListener('change', submitFilter);
	}
});
</script>
