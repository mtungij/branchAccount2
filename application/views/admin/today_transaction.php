...
<a href="<?php echo base_url("admin/delete_depost_data/{$cashs->pay_id}") ?>" class="delete-transaction-link py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-red-600 text-white hover:bg-red-700 focus:outline-hidden focus:bg-red-700 disabled:opacity-50 disabled:pointer-events-none">
...
document.querySelectorAll('.delete-transaction-link').forEach(function(link) {
  link.addEventListener('click', function() {
    ...
    const icon = link.querySelector('.delete-icon');
    const label = link.querySelector('.delete-label');
    const loader = link.querySelector('.delete-loader');
