...
<div class="w-full lg:ps-64">
  ...
  <div>
            ...
            <?php echo $this->lang->line('today_received'); ?> -(<?php echo $blanch_data->blanch_name; ?>) <?php echo $this->lang->line('from'); ?> <?php echo $from; ?> - <?php echo $this->lang->line('to'); ?> <?php echo $to; ?> 
                <?php if ($empl_id == 'all' || empty($empl_data) || empty($empl_data->empl_name)) {
				 ?>
...
<?php if (count($data_blanch) > 0): ?>
    <a href="<?php echo base_url('admin/print_received_deposit?from=' . urlencode($from) . '&to=' . urlencode($to) . '&blanch_id=' . urlencode($blanch_id) . '&empl_id=' . urlencode($empl_id) . '&dep_status=' . urlencode((string) $dep_status)); ?>" 
       target="_blank"
